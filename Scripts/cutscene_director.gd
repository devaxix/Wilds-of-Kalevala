...
# --- UI NODES (UPDATED PATHS) ---
# We now look inside the "EndingUI_Layer" first
@onready var ending_ui = $"../EndingUI_Layer/EndingUI"
@onready var choice_container = $"../EndingUI_Layer/EndingUI/ChoiceContainer"
@onready var credits_container = $"../EndingUI_Layer/EndingUI/CreditsContainer"

# --- BUTTONS (UPDATED PATHS) ---
@onready var btn_a = $"../EndingUI_Layer/EndingUI/ChoiceContainer/VBoxContainer/ButtonA"
@onready var btn_b = $"../EndingUI_Layer/EndingUI/ChoiceContainer/VBoxContainer/ButtonB"
@onready var btn_c = $"../EndingUI_Layer/EndingUI/ChoiceContainer/VBoxContainer/ButtonC"
...
func start_cutscene():
	...
	# 7. SHOW CHOICES (Simpler now!)
	# No position math needed! The CanvasLayer holds it in place.
	
	choice_container.visible = true
	
	# Fade In Tween
	var fade_in = create_tween()
	fade_in.tween_property(choice_container, "modulate:a", 1.0, 1.5)

func _on_choice_picked():
	# 1. STOP CLICKING (Without turning them ugly/transparent)
	# We disconnect the signal so clicking again does nothing.
	if btn_a.pressed.is_connected(_on_choice_picked): btn_a.pressed.disconnect(_on_choice_picked)
	if btn_b.pressed.is_connected(_on_choice_picked): btn_b.pressed.disconnect(_on_choice_picked)
	if btn_c.pressed.is_connected(_on_choice_picked): btn_c.pressed.disconnect(_on_choice_picked)
	
	# 2. FADE OUT (Smoothly)
	var fade_out = create_tween()
	fade_out.tween_property(choice_container, "modulate:a", 0.0, 1.0)
